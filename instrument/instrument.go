...
func (p *prominstrument) ProxiedBytes(ctx context.Context, sent, recv int, platform, version, app, locale, dataCapCohort string, clientIP net.IP, deviceID, originHost string) {
	...
	coreOtelAttributes := []attribute.KeyValue{
	...
	otelAttributes := func(attributes ...attribute.KeyValue) []attribute.KeyValue {
		allAttributes := make([]attribute.KeyValue, 0, len(coreOtelAttributes)+len(attributes))
		allAttributes = append(allAttributes, coreOtelAttributes...)
		allAttributes = append(allAttributes, attributes...)
		return allAttributes
	}
	otelinstrument.Data.Add(
		ctx,
		int64(sent),
		otelAttributes(attribute.KeyValue{"direction", attribute.StringValue("transmit")})...,
	)
	otelinstrument.Data.Add(
		ctx,
		int64(recv),
		otelAttributes(attribute.KeyValue{"direction", attribute.StringValue("receive")})...,
	)
...
func (s *stats) OnRecv(n uint64) {
	...
	otelinstrument.MPFrames.Add(context.Background(), 1, s.otelAttributes(attribute.KeyValue{"state", attribute.StringValue("received")})...)
	otelinstrument.MPData.Add(context.Background(), int64(n), s.otelAttributes(attribute.KeyValue{"state", attribute.StringValue("received")})...)
}

func (s *stats) OnSent(n uint64) {
	s.framesSent.Inc()
	s.bytesSent.Add(float64(n))
	otelinstrument.MPFrames.Add(context.Background(), 1, s.otelAttributes(attribute.KeyValue{"state", attribute.StringValue("sent")})...)
	otelinstrument.MPData.Add(context.Background(), int64(n), s.otelAttributes(attribute.KeyValue{"state", attribute.StringValue("sent")})...)
}

func (s *stats) OnRetransmit(n uint64) {
	s.framesRetransmitted.Inc()
	s.bytesRetransmitted.Add(float64(n))
	otelinstrument.MPFrames.Add(context.Background(), 1, s.otelAttributes(attribute.KeyValue{"state", attribute.StringValue("retransmitted")})...)
	otelinstrument.MPData.Add(context.Background(), int64(n), s.otelAttributes(attribute.KeyValue{"state", attribute.StringValue("retransmitted")})...)
}
...
func (s *stats) otelAttributes(attributes ...attribute.KeyValue) []attribute.KeyValue {
	allAttributes := make([]attribute.KeyValue, 0, len(s.coreOtelAttributes)+len(attributes))
	allAttributes = append(allAttributes, s.coreOtelAttributes...)
	allAttributes = append(allAttributes, attributes...)
	return allAttributes
}
...
func (prom *prominstrument) MultipathStats(protocols []string) (trackers []multipath.StatsTracker) {
	for _, p := range protocols {
		...
		trackers = append(trackers, &stats{
			...
			coreOtelAttributes:  []attribute.KeyValue{attribute.KeyValue{"path_protocol", attribute.StringValue(p)}},
		})
	}
	return
}
