...
func doTestThrottling(t *testing.T, pro bool, serverAddr string, redisIsUp bool, throttleThreshold, throttleRate int, r testredis.Redis) {
	...
	if redisIsUp {
		...
		require.NoError(t, rc.Set(context.Background(), "_throttle", settings, 0).Err())
	}
	...
	result, err := rc.HMGet(context.Background(), "_client:"+deviceId, "bytesIn", "bytesOut", "countryCode", "clientIP").Result()
