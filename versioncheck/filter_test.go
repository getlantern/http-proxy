...
func TestShouldRedirect(t *testing.T) {
	f := (New("3.1.1", "http://versioncheck.com/badversion", 1).(*filter))
	req, _ := http.NewRequest("POST", "http://anysite.com", nil)
	assert.False(t, f.shouldRedirect(req), "should only redirect GET requests")
	req, _ = http.NewRequest("GET", "http://anysite.com", nil)
	assert.False(t, f.shouldRedirect(req), "should only redirect HTML requests")
...
func testPercentage(t *testing.T, percentage float64, exact bool) {
	f := (New("3.1.1", "http://versioncheck.com/badversion", percentage).(*filter))
	req, _ := http.NewRequest("GET", "http://anysite.com", nil)
	req.Header.Set("Accept", "text/html")
	req.Header.Set("User-Agent", "Mozilla/5.0 xxx")
	hit := 0
