...
var (
	log = zaplog.LoggerFor("http-proxy-lantern.analytics")
)
...
func New(opts *Options) filters.Filter {
	...
	log.Debugf("Will report analytics as %v using hostname '%v', sampling %d percent of requests", eng.GetID(), hostname, int(opts.SamplePercentage*100))
...
func (am *analyticsMiddleware) trackSession(args string) {
	...
	log.Debugf("Successfully sent request to analytics engine: %s", resp.Status)
