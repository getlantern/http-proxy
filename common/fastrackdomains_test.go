...
func TestWhitelist(t *testing.T) {
	...
	wl := NewRawFasttrackDomains(domains)
	...
	req, _ = http.NewRequest("GET", "http://geo.getiantem.org:80", nil)
	assert.True(t, wl.Whitelisted(req))

	req, _ = http.NewRequest("GET", "http://agetiantem.org", nil)
